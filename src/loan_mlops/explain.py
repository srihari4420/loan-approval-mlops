...
Used both globally (which features drive predictions overall) and locally
(why was this specific application rejected). Regulators care about the local one.
"""
...
def _transform(pipeline: Pipeline, X: pd.DataFrame) -> tuple[np.ndarray, list[str]]:
    """Run X through the preprocessor and return the array + feature names."""
...
def compute_shap_values(
    ...
    Uses TreeExplainer for tree models; falls back to KernelExplainer otherwise.
    For large datasets, samples down to `sample_size` rows — KernelExplainer is slow.
    """
    ...
    # TreeExplainer is exact and fast for XGBoost/LightGBM/RandomForest
    if hasattr(clf, "get_booster") or clf.__class__.__name__ in {"XGBClassifier", "LGBMClassifier"}:
        explainer = shap.TreeExplainer(clf)
        values = explainer(transformed)
    else:
        # Linear/kernel fallback — slower but works for LogReg etc.
...
def explain_single(
    ...
    """Per-prediction explanation — what drove the score for ONE applicant.

    Returns the kind of thing you'd actually surface to a credit officer:
      - the score
      - the top k features pushing the score up (risk factors)
      - the top k features pushing it down (protective factors)
    """
...
def save_global_summary(
    ...
    """Save the SHAP summary plot — global feature importance with directionality."""
